...
- When working on an issue, checkout a branch matching the issue ID in lowercase (e.g. `{{team_lower}}-259` for `{{team}}-259`). The CLI auto-detects the linked issue from the branch name for commands like `issue view`, `issue update`, and `comment add`.
