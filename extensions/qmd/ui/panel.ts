...
export class QmdPanel {
	...
	render(width: number): string[] {
		...
		// ── Bottom border ───────────────────────────────────
		const bot_border = truncateToWidth(`${bdr("╰")}${bdr("─".repeat(footer_inner_w))}${bdr("╯")}`, w);
	...
	private render_sidebar(width: number, height: number): string[] {
		...
		const visible = entries.slice(this.sidebar_scroll_offset, this.sidebar_scroll_offset + list_height - 1);
		for (let i = 0; i < visible.length; i++) {
			...
			if (entry.type === "all") {
				...
				lines.push(truncateToWidth(` ${marker} ${name}${" ".repeat(gap)}${bound} ${t.fg("dim", count)}`, width));
			}
		}
	...
	private get_sidebar_entries(): Array<{ type: "all" } | { type: "collection"; collection: QmdCollectionSummary }> {
		const query = this.sidebar_filter_query.trim().toLowerCase();
		const entries: Array<{ type: "all" } | { type: "collection"; collection: QmdCollectionSummary }> = [];
	...
	private render_main_overview(width: number, height: number): string[] {
		...
		if (snap.binding_status === "unavailable") {
			...
			if (snap.contexts.length > 0) {
				...
				content.push("");
				const max_path_len = Math.min(20, Math.max(6, ...snap.contexts.map((ctx) => ctx.path.length)));
		...
		const visible = content.slice(this.overview_scroll_offset, this.overview_scroll_offset + height);
	...
	private render_main_files(width: number, height: number): string[] {
		...
		const visible_entries = this.tree_flat.slice(this.tree_scroll_offset, this.tree_scroll_offset + tree_area_h);
