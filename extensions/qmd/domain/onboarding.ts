...
import { collection_key_from_repo_root, read_repo_marker, write_repo_marker } from "./repo-binding.js";
...
export function build_draft_proposal(scan: RepoScan): DraftInitProposal {
	const paths = scan.directories
		.filter((directory) => directory.markdown_file_count > 0 || directory.path in CONTEXT_HEURISTICS)
		.map((directory) => ({
			path: directory.path,
			annotation: CONTEXT_HEURISTICS[directory.path] ?? `${directory.path} documentation and markdown content.`,
		}))
		.sort((left, right) => compare_paths(left, right));
...
export function build_init_prompt(scan: RepoScan, draft: DraftInitProposal): string {
	const key_files = scan.key_files.length > 0 ? scan.key_files.map((file) => `- ${file}`).join("\n") : "- (none found)";
	const sample_paths =
		scan.sample_paths.length > 0 ? scan.sample_paths.map((file) => `- ${file}`).join("\n") : "- (none found)";
	const project_shape =
		scan.project_shape_hints.length > 0 ? scan.project_shape_hints.join(", ") : "(no common QMD heuristics matched)";
	const draft_paths =
...
export async function normalize_init_proposal(
	...
	if (!parsed.success) {
		throw new InvalidInitProposalError(parsed.error.issues.map((issue) => issue.message).join("; "), parsed.error);
	}
	...
	for (const entry of parsed.data.paths) {
		...
		if (!annotation) {
			throw new InvalidInitProposalError(`Context path '${entry.path || "/"}' must have a non-empty annotation.`);
		}
