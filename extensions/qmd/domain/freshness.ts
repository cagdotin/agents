...
export async function check_freshness(marker: QmdRepoMarker): Promise<FreshnessResult> {
	...
	try {
		...
		return {
			status: "unknown",
			reason: "This repository is not a git worktree, so freshness could not be derived from commits.",
		};
	}
	...
	try {
		const [{ stdout: tracked_stdout }, { stdout: untracked_stdout }] = await Promise.all([
			...
			exec_file("git", ["-C", marker.repo_root, "ls-files", "--others", "--exclude-standard", "--", "*.md", "**/*.md"]),
		]);
