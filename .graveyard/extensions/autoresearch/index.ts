...
function renderDashboardLines(st: ExperimentState, width: number, th: Theme, maxRows: number = 6): string[] {
	const lines: string[] = [];
	...
	if (bestPrimary !== null) {
		...
		if (st.secondaryMetrics.length > 0) {
			...
			if (secParts.length > 0) {
				lines.push(truncateToWidth(`  ${th.fg("dim", "          ")}${th.fg("muted", secParts.join("  "))}`, width));
			}
		}
	}
	...
	const secMetrics = st.secondaryMetrics.filter((sm) => visibleRows.some((r) => r.metrics?.[sm.name] !== undefined));

	// Column definitions
	const col = { idx: 3, commit: 8, primary: 11, status: 8 };
	const secColWidth = 11;
	const totalSecWidth = secMetrics.length * secColWidth;
	const descW = Math.max(10, width - col.idx - col.commit - col.primary - totalSecWidth - col.status - 6);
	...
	headerLine += `${th.fg("muted", "status".padEnd(col.status))}` + `${th.fg("muted", "description")}`;
	...
	const baselineSecondary = findBaselineSecondary(st.results, st.currentSegment, st.secondaryMetrics);

	// Show max 6 recent runs, with a note about hidden earlier ones
	if (startIdx > 0) {
		lines.push(truncateToWidth(`  ${th.fg("dim", `… ${startIdx} earlier run${startIdx === 1 ? "" : "s"}`)}`, width));
	}
	...
	for (let i = startIdx; i < st.results.length; i++) {
		...
		const color = isOld ? "dim" : r.status === "keep" ? "success" : r.status === "crash" ? "error" : "warning";
		...
		rowLine += `${th.fg(color, r.status.padEnd(col.status))}` + `${th.fg("muted", r.description.slice(0, descW))}`;
...
export default function autoresearchExtension(pi: ExtensionAPI) {
	...
	pi.registerTool({
		...
		renderResult(result, { expanded, isPartial }, theme) {
			...
			let text = theme.fg("success", "✅ ") + theme.fg("accent", `${d.durationSeconds.toFixed(1)}s`);
	...
	pi.registerTool({
		...
		renderCall(args, theme) {
			...
			const color = args.status === "keep" ? "success" : args.status === "crash" ? "error" : "warning";
		...
		renderResult(result, _options, theme) {
			...
			const color = exp.status === "keep" ? "success" : exp.status === "crash" ? "error" : "warning";
			...
			if (s.bestMetric !== null) {
				text += theme.fg("dim", " │ ") + theme.fg("warning", theme.bold(`★ ${formatNum(s.bestMetric, s.metricUnit)}`));
			}
				...
				(tui, theme, _kb, done) => {
					...
					return {
						render(width: number): string[] {
							...
							out.push(
								truncateToWidth(theme.fg("borderMuted", "─".repeat(footFill)) + theme.fg("dim", helpText), width),
							);
