...
Design and build a visual design review loop — agent finishes FE implementation, browser opens with review overlay, human annotates elements with feedback, agent receives structured feedback and iterates. Works locally (cmux + ephemeral server) and remotely (ODE + persistent service).
