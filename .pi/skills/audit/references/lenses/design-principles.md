...
Evaluates code structure and design decisions against the Unix philosophy.

Full source and context: `docs/resources/unix-philosophy-raymond.md`
...
- Read the code in scope. For each rule, ask: does this code follow or violate it?
- Not every rule applies to every scope. Use judgment — skip rules that don't apply.
- Only report findings where there is a concrete observation. Omit clean passes.

## Severity

- **concern** — design smell worth noting; address when next in this code
- **violation** — clear problem with maintainability or usability impact; fix soon
